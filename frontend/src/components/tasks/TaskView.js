...
const TaskView = ({ openTask, setOpenTask }) => {
  ...
  const { isTaskLoading, task, isRequesting } = useSelector(
  ...
  const handleDelete = () => {
    dispatch(deleteTask({ boardId: task.board.id, taskId: task.id }));
  };
          ...
          <DialogContent classes={{ root: classes.dialogContentRoot }}>
            <Grid container className={classes.root}>
              <Grid item xs={12} sm={8} className={classes.main}>
                ...
                <Divider />
                <Box sx={{ my: 1 }}>
                  <Typography className={classes.subHeader} component="h5">
                    Comments
                  </Typography>
                  <Box sx={{ my: 2 }}>
              ...
              <Grid item xs={12} sm={4} className={classes.sideBar}>
                {task.deadline && (
                  <Chip
                    icon={<Alarm />}
                    label={`task due ${timeDiffFromNow(task.deadline)}`}
                  />
                )}
                <Button
                  size="small"
                ...
                <Box sx={{ my: 2 }}>
                  <Typography className={classes.subHeader} component="h2">
                    Members
                  </Typography>
                  <TasksMembers
                    id={task.id}
