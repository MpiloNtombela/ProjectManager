...
export const register = (newUser, history) => dispatch => {
    ...
    axios.post('/api/auth/registration/', body, config)
        .then((res) => {
...
export const confirmEmail = (key, history) => dispatch => {
    ...
    axios.post('/api/auth/account-confirm-email/', body, config)
        .then(() => {
        ...
        .catch(err => {
            batch(() => {
                ...
                dispatch(createSnackAlert('Verification failed. Either link is corrupt or expired', err.response.status))
            })
        })
}
export const resendConfirmEmail = (email, history) => dispatch => {
    ...
    axios.post('/api/auth/resend-account-confirmation-email/', body, config)
        .then((res) => {
            batch(() => {
...
export const forgotPassword = email => dispatch => {
    ...
    axios.post('/api/auth/password/reset/', body, config)
        .then((res) => {
            dispatch({
...
export const passwordReset = (credentials, history) => dispatch => {
    ...
    // Request Body
    const body = JSON.stringify(credentials);
    axios.post('/api/auth/password/reset/confirm/', body, config)
        .then((res) => {
            dispatch({
...
export const passwordChange = (credentials, history) => (dispatch, getState) => {
    ...
    // Request Body
    const body = JSON.stringify(credentials);
    axios.post('/api/auth/password/change/', body, tokenConfig(getState))
        .then((res) => {
            dispatch({
